...
import org.graalvm.compiler.core.i386.I386NodeMatchRules;
...
import org.graalvm.compiler.lir.i386.I386FrameMap;
 ...
 */
public class I386HotSpotBackend extends HotSpotHostBackend {
    ...
    public FrameMapBuilder newFrameMapBuilder(RegisterConfig registerConfig) {
        ...
        return new I386FrameMapBuilder(newFrameMap(registerConfigNonNull), getCodeCache(), registerConfigNonNull);
    }

    @Override
    public FrameMap newFrameMap(RegisterConfig registerConfig) {
        return new I386FrameMap(getCodeCache(), registerConfig, this);
    }

    @Override
    public LIRGeneratorTool newLIRGenerator(LIRGenerationResult lirGenRes) {
        return new I386HotSpotLIRGenerator(getProviders(), config, lirGenRes);
    }
    ...
    public NodeLIRBuilderTool newNodeLIRBuilder(StructuredGraph graph, LIRGeneratorTool lirGen) {
        return new I386HotSpotNodeLIRBuilder(graph, lirGen, new I386NodeMatchRules(lirGen));
    }

    @Override
    protected void bangStackWithOffset(CompilationResultBuilder crb, int bangOffset) {
        I386MacroAssembler asm = (I386MacroAssembler) crb.asm;
        int pos = asm.position();
        asm.movl(new I386Address(rsp, -bangOffset), I386.rax);
            ...
            if (omitFrame) {
                ...
                if (ZapStackOnMethodEntry.getValue(crb.getOptions())) {
                    ...
                    for (int i = 0; i < frameSize / intSize; ++i) {
                        asm.movl(new I386Address(rsp, i * intSize), 0xC1C1C1C1);
                    }
                }
    ...
    protected Assembler createAssembler(FrameMap frameMap) {
        if (((I386) getTarget().arch).getFeatures().contains(CPUFeature.AVX)) {
            return new I386VectorAssembler(getTarget());
        } else {
            return new I386MacroAssembler(getTarget());
        }
    }
     ...
     */
    public void emitCodePrefix(ResolvedJavaMethod installedCodeOwner, CompilationResultBuilder crb, I386MacroAssembler asm, RegisterConfig regConfig, Label verifiedEntry) {
        ...
        if (installedCodeOwner != null && !installedCodeOwner.isStatic()) {
            ...
            I386Address src = new I386Address(receiver, config.hubOffset);

            if (config.useCompressedClassPointers) {
                Register register = r10;
                I386HotSpotMove.decodeKlassPointer(crb, asm, register, providers.getRegisters().getHeapBaseRegister(), src, config);
            ...
            I386Call.directConditionalJmp(crb, asm, getForeignCalls().lookupForeignCall(IC_MISS_HANDLER), ConditionFlag.NotEqual);
        }
        ...
        if (GeneratePIC.getValue(crb.getOptions())) {
            ...
            if (!frameContext.isStub) {
                crb.recordInlineDataInCodeWithNote(new HotSpotSentinelConstant(LIRKind.value(I386Kind.QWORD), JavaKind.Long), HotSpotConstantLoadAction.MAKE_NOT_ENTRANT);
                asm.movq(I386.rax, asm.getPlaceholder(-1));
                asm.testq(I386.rax, I386.rax);
                I386Call.directConditionalJmp(crb, asm, getForeignCalls().lookupForeignCall(WRONG_METHOD_HANDLER), ConditionFlag.NotZero);
            }
        }
    }
     ...
     * @param installedCodeOwner see {@link Backend#emitCode}
     */
    public void emitCodeSuffix(ResolvedJavaMethod installedCodeOwner, CompilationResultBuilder crb, I386MacroAssembler asm, FrameMap frameMap) {
        HotSpotProviders providers = getProviders();
        HotSpotFrameContext frameContext = (HotSpotFrameContext) crb.frameContext;
        if (!frameContext.isStub) {
            HotSpotForeignCallsProvider foreignCalls = providers.getForeignCalls();
            crb.recordMark(config.MARKID_EXCEPTION_HANDLER_ENTRY);
            I386Call.directCall(crb, asm, foreignCalls.lookupForeignCall(EXCEPTION_HANDLER), null, false, null);
            crb.recordMark(config.MARKID_DEOPT_HANDLER_ENTRY);
            I386Call.directCall(crb, asm, foreignCalls.lookupForeignCall(DEOPTIMIZATION_HANDLER), null, false, null);
        } else {
    ...
    public RegisterAllocationConfig newRegisterAllocationConfig(RegisterConfig registerConfig, String[] allocationRestrictedTo) {
        RegisterConfig registerConfigNonNull = registerConfig == null ? getCodeCache().getRegisterConfig() : registerConfig;
        return new I386HotSpotRegisterAllocationConfig(registerConfigNonNull, allocationRestrictedTo);
    }
