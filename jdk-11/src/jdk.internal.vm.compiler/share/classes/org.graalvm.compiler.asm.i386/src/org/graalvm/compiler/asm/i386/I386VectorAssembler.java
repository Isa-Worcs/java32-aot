...
/**
 * This class extends the I386 assembler with functions that emit instructions from the AVX
 * extension.
 */
public class I386VectorAssembler extends I386MacroAssembler {

    public I386VectorAssembler(TargetDescription target) {
        super(target);
        assert ((I386) target.arch).getFeatures().contains(CPUFeature.AVX);
    }
    ...
    private void emitVEX(int l, int pp, int mmmmm, int w, int rxb, int vvvv) {
        assert ((I386) target.arch).getFeatures().contains(CPUFeature.AVX) : "emitting VEX prefix on a CPU without AVX support";
     ...
     * Format: [VEX] [Opcode] [ModR/M] [Imm8[7:4]]
     */
    private void emitVexOp(int l, int pp, int mmmmm, int w, int op, Register reg, Register vvvv, I386Address rm, Register imm8, int additionalInstructionSize) {
        emitVEX(l, pp, mmmmm, w, getRXB(reg, rm), vvvv.encoding());
        emitByte(op);
     ...
     */
    private void emitVexOp(int l, int pp, int mmmmm, int w, int op, Register reg, I386Address rm, int additionalInstructionSize) {
        emitVEX(l, pp, mmmmm, w, getRXB(reg, rm), 0);
        emitByte(op);
        emitOperandHelper(reg, rm, additionalInstructionSize);
    }
     ...
     * Format: [VEX] [Opcode] [ModR/M] [SIB] [Disp]
     */
    private void emitVexOp(int l, int pp, int mmmmm, int w, int op, Register reg, Register vvvv, I386Address rm, int additionalInstructionSize) {
        emitVEX(l, pp, mmmmm, w, getRXB(reg, rm), vvvv.encoding());
        emitByte(op);
        emitOperandHelper(reg, rm, additionalInstructionSize);
    }
    ...
    private static final OpAssertion XMM_CPU = new OpAssertion(CPUFeature.AVX, null, I386.XMM, null, I386.CPU, null);
    private static final OpAssertion XMM_XMM_CPU = new OpAssertion(CPUFeature.AVX, null, I386.XMM, I386.XMM, I386.CPU, null);
    private static final OpAssertion CPU_XMM = new OpAssertion(CPUFeature.AVX, null, I386.CPU, null, I386.XMM, null);
    ...
    private static final class OpAssertion {
        ...
        private OpAssertion(CPUFeature avx128feature, CPUFeature avx256feature) {
            this(avx128feature, avx256feature, I386.XMM, I386.XMM, I386.XMM, I386.XMM);
        }
        ...
        public boolean check(I386 arch, AVXSize size, Register r, Register v, Register m) {
            return check(arch, size, r, v, m, null);
        }

        public boolean check(I386 arch, AVXSize size, Register r, Register v, Register m, Register imm8) {
            switch (size) {
                case XMM:
    ...
    private static class VexOp {
        ...
        public boolean isSupported(I386VectorAssembler vasm, I386Kind kind) {
            return assertion.supports(((I386) vasm.target.arch).getFeatures(), AVXKind.getRegisterSize(kind));
        }
    ...
    public static class VexRROp extends VexOp {
        ...
        public void emit(I386VectorAssembler asm, AVXSize size, Register dst, Register src) {
            assert assertion.check((I386) asm.target.arch, size, dst, null, src);
    ...
    public static class VexRMOp extends VexRROp {
        ...
        public void emit(I386VectorAssembler asm, AVXSize size, Register dst, I386Address src) {
            assert assertion.check((I386) asm.target.arch, size, dst, null, null);
    ...
    public static final class VexMoveOp extends VexRMOp {
        ...
        public void emit(I386VectorAssembler asm, AVXSize size, I386Address dst, Register src) {
            assert assertion.check((I386) asm.target.arch, size, src, null, null);
            asm.emitVexOp(getLFlag(size), pp, mmmmm, w, opReverse, src, dst, 0);
        }

        public void emitReverse(I386VectorAssembler asm, AVXSize size, Register dst, Register src) {
            assert assertion.check((I386) asm.target.arch, size, src, null, dst);
            asm.emitVexOp(getLFlag(size), pp, mmmmm, w, opReverse, src, dst);
        }
    }

    public interface VexRRIOp {
        void emit(I386VectorAssembler asm, AVXSize size, Register dst, Register src, int imm8);
    }
    ...
    public static final class VexRMIOp extends VexOp implements VexRRIOp {
        ...
        @Override
        public void emit(I386VectorAssembler asm, AVXSize size, Register dst, Register src, int imm8) {
            assert assertion.check((I386) asm.target.arch, size, dst, null, src);
            asm.emitVexOp(getLFlag(size), pp, mmmmm, w, op, dst, src);
            asm.emitByte(imm8);
        }

        public void emit(I386VectorAssembler asm, AVXSize size, Register dst, I386Address src, int imm8) {
            assert assertion.check((I386) asm.target.arch, size, dst, null, null);
    ...
    public static final class VexMRIOp extends VexOp implements VexRRIOp {
        ...
        public void emit(I386VectorAssembler asm, AVXSize size, I386Address dst, Register src, int imm8) {
            assert assertion.check((I386) asm.target.arch, size, src, null, null);
    ...
    public static class VexRVMROp extends VexOp {
        ...
        public void emit(I386VectorAssembler asm, AVXSize size, Register dst, Register mask, Register src1, Register src2) {
            assert assertion.check((I386) asm.target.arch, size, dst, mask, src1, src2);
            asm.emitVexOp(getLFlag(size), pp, mmmmm, w, op, dst, src1, src2, mask);
        }

        public void emit(I386VectorAssembler asm, AVXSize size, Register dst, Register mask, Register src1, I386Address src2) {
            assert assertion.check((I386) asm.target.arch, size, dst, mask, src1, null);
    ...
    public static class VexRVMOp extends VexOp {
        ...
        public void emit(I386VectorAssembler asm, AVXSize size, Register dst, Register src1, Register src2) {
            assert assertion.check((I386) asm.target.arch, size, dst, src1, src2);
            asm.emitVexOp(getLFlag(size), pp, mmmmm, w, op, dst, src1, src2);
        }

        public void emit(I386VectorAssembler asm, AVXSize size, Register dst, Register src1, I386Address src2) {
            assert assertion.check((I386) asm.target.arch, size, dst, src1, null);
    ...
    public static final class VexShiftOp extends VexRVMOp implements VexRRIOp {
        ...
        @Override
        public void emit(I386VectorAssembler asm, AVXSize size, Register dst, Register src, int imm8) {
            assert assertion.check((I386) asm.target.arch, size, null, dst, src);
    ...
    public static final class VexMaskMoveOp extends VexOp {
        ...
        public void emit(I386VectorAssembler asm, AVXSize size, Register dst, Register mask, I386Address src) {
            assert assertion.check((I386) asm.target.arch, size, dst, mask, null);
            asm.emitVexOp(getLFlag(size), pp, mmmmm, w, op, dst, mask, src, 0);
        }

        public void emit(I386VectorAssembler asm, AVXSize size, I386Address dst, Register mask, Register src) {
            assert assertion.check((I386) asm.target.arch, size, src, mask, null);
    ...
    public static final class VexRVMIOp extends VexOp {
        ...
        public void emit(I386VectorAssembler asm, AVXSize size, Register dst, Register src1, Register src2, int imm8) {
            assert assertion.check((I386) asm.target.arch, size, dst, src1, src2);
        ...
        public void emit(I386VectorAssembler asm, AVXSize size, Register dst, Register src1, I386Address src2, int imm8) {
            assert assertion.check((I386) asm.target.arch, size, dst, src1, null);
            assert (imm8 & 0xFF) == imm8;
    ...
    public static final class VexFloatCompareOp extends VexOp {
        ...
        public void emit(I386VectorAssembler asm, AVXSize size, Register dst, Register src1, Register src2, Predicate p) {
            assert assertion.check((I386) asm.target.arch, size, dst, src1, src2);
            asm.emitVexOp(getLFlag(size), pp, mmmmm, w, op, dst, src1, src2);
            asm.emitByte(p.imm8);
        }

        public void emit(I386VectorAssembler asm, AVXSize size, Register dst, Register src1, I386Address src2, Predicate p) {
            assert assertion.check((I386) asm.target.arch, size, dst, src1, null);
            asm.emitVexOp(getLFlag(size), pp, mmmmm, w, op, dst, src1, src2, 1);
            asm.emitByte(p.imm8);
        }
    ...
    @Override
    public void movflt(Register dst, I386Address src) {
        VexMoveOp.VMOVSS.emit(this, AVXSize.XMM, dst, src);
    }

    @Override
    public void movflt(I386Address dst, Register src) {
        VexMoveOp.VMOVSS.emit(this, AVXSize.XMM, dst, src);
    }
    ...
    @Override
    public void movdbl(Register dst, I386Address src) {
        VexMoveOp.VMOVSD.emit(this, AVXSize.XMM, dst, src);
    }

    @Override
    public void movdbl(I386Address dst, Register src) {
        VexMoveOp.VMOVSD.emit(this, AVXSize.XMM, dst, src);
    }
}
