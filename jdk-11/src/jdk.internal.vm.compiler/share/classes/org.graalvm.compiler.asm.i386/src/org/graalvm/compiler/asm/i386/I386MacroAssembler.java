...
import static org.graalvm.compiler.asm.i386.I386AsmOptions.UseIncDec;
import static org.graalvm.compiler.asm.i386.I386AsmOptions.UseXmmLoadAndClearUpper;
import static org.graalvm.compiler.asm.i386.I386AsmOptions.UseXmmRegToRegMoveAll;
 ...
 */
public class I386MacroAssembler extends I386Assembler {

    public I386MacroAssembler(TargetDescription target) {
        super(target);
    }
    ...
    public final void decrementq(I386Address dst, int value) {
        if (value == Integer.MIN_VALUE) {
            subq(dst, value);
            return;
        }
    ...
    public final void incrementq(I386Address dst, int value) {
        if (value == Integer.MIN_VALUE) {
            addq(dst, value);
            return;
        }
    ...
    public final void movptr(Register dst, I386Address src) {
        movq(dst, src);
    }

    public final void movptr(I386Address dst, Register src) {
        movq(dst, src);
    }

    public final void movptr(I386Address dst, int src) {
        movslq(dst, src);
    }
    ...
    public final void cmpptr(Register src1, I386Address src2) {
        cmpq(src1, src2);
    }
    ...
    public final void decrementl(I386Address dst, int value) {
        if (value == Integer.MIN_VALUE) {
            subl(dst, value);
            return;
        }
    ...
    public final void incrementl(I386Address dst, int value) {
        if (value == Integer.MIN_VALUE) {
            addl(dst, value);
            return;
        }
     ...
     * volatile field!
     */
    public final void movlong(I386Address dst, long src) {
        if (NumUtil.isInt(src)) {
            I386MIOp.MOV.emit(this, OperandSize.QWORD, dst, (int) src);
        } else {
            I386Address high = new I386Address(dst.getBase(), dst.getIndex(), dst.getScale(), dst.getDisplacement() + 4);
    ...
    public final void flog(Register dest, Register value, boolean base10) {
        ...
        I386Address tmp = trigPrologue(value);
        fyl2x();
    ...
    private I386Address trigPrologue(Register value) {
        assert value.getRegisterCategory().equals(I386.XMM);
        I386Address tmp = new I386Address(I386.rsp);
        subq(I386.rsp, I386Kind.DOUBLE.getSizeInBytes());
        movdbl(tmp, value);
        fldd(tmp);
        return tmp;
    }

    private void trigEpilogue(Register dest, I386Address tmp) {
        assert dest.getRegisterCategory().equals(I386.XMM);
        fstpd(tmp);
        movdbl(dest, tmp);
        addq(I386.rsp, I386Kind.DOUBLE.getSizeInBytes());
    }
    ...
    public void stringIndexofC8(Register str1, Register str2,
        ...
        // Load substring.
        movdqu(vec, new I386Address(str2, 0));
        movl(cnt2, intCnt2);
        ...
        bind(scanToSubstr);
        pcmpestri(vec, new I386Address(result, 0), 0x0d);
        ...
        leaq(result, new I386Address(result, tmp, Scale.Times2, 0));
        ...
        if (intCnt2 > 8) {
            ...
            if (intCnt2 < 1024 * 1024 * 1024) {
                movdqu(vec, new I386Address(str2, cnt2, Scale.Times2, intCnt2 * 2));
                pcmpestri(vec, new I386Address(result, cnt2, Scale.Times2, intCnt2 * 2), 0x0d);
            } else {
                // calculate index in register to avoid integer overflow (int_cnt2*2)
                movl(tmp, intCnt2);
                addq(tmp, cnt2);
                movdqu(vec, new I386Address(str2, tmp, Scale.Times2, 0));
                pcmpestri(vec, new I386Address(result, tmp, Scale.Times2, 0), 0x0d);
            }
    ...
    public void stringIndexOf(Register str1, Register str2,
        ...
        if (intCnt2 > 0) {     // small (< 8 chars) constant substring
            if (intCnt2 == 1) {  // One char
                movzwl(result, new I386Address(str2, 0));
                movdl(vec, result); // move 32 bits
            } else if (intCnt2 == 2) { // Two chars
                movdl(vec, new I386Address(str2, 0)); // move 32 bits
            } else if (intCnt2 == 4) { // Four chars
                movq(vec, new I386Address(str2, 0));  // move 64 bits
            } else { // cnt2 = { 3, 5, 6, 7 }
                // Array header size is 12 bytes in 32-bit VM
                // + 6 bytes for 3 chars == 18 bytes,
                // enough space to load vec and shift.
                movdqu(vec, new I386Address(str2, (intCnt2 * 2) - 16));
            ...
            bind(copySubstr);
            movzwl(result, new I386Address(str2, cnt2, Scale.Times2, -2));
            movw(new I386Address(rsp, cnt2, Scale.Times2, stackOffset), result);
            decrementl(cnt2, 1);
        ...
        bind(copyStr);
        movzwl(result, new I386Address(str1, cnt2, Scale.Times2, -2));
        movw(new I386Address(rsp, cnt2, Scale.Times2, stackOffset), result);
        decrementl(cnt2, 1);
        ...
        if (intCnt2 < 0) {  // Only for non constant substring
            ...
            // is executed only for large substrings (> 8 chars)
            bind(reloadSubstr);
            movq(str2, new I386Address(rsp, 2 * wordSize));
            movl(cnt2, new I386Address(rsp, 3 * wordSize));
            movdqu(vec, new I386Address(str2, 0));
            // We came here after the beginning of the substring was
            // matched but the rest of it was not so we need to search
            // again. Start from the next element after the previous match.
        ...
        leaq(result, new I386Address(result, cnt1, Scale.Times2, -16));
        movl(cnt1, 8);
        jmpb(scanToSubstr);
        ...
        bind(foundSubstr);
        // Compute start addr of substr
        leaq(result, new I386Address(result, tmp, Scale.Times2));
        ...
        if (intCnt2 > 0) { // Constant substring
            ...
            bind(scanSubstr);
            pcmpestri(vec, new I386Address(str1, 0), 0x0d);
            // Need to reload strings pointers if not matched whole vector
            jcc(ConditionFlag.NoOverflow, reloadSubstr); // OF == 0
            ...
            // Back-up strings to avoid reading beyond substring.
            leaq(str2, new I386Address(str2, cnt2, Scale.Times2, -16));
            leaq(str1, new I386Address(str1, cnt2, Scale.Times2, -16));
            subl(cnt1, cnt2);
            movl(cnt2, 8);
            addl(cnt1, 8);
            bind(contScanSubstr);
            ...
            bind(retFoundLong);
            movq(str1, new I386Address(rsp, wordSize));
        } // non constant
