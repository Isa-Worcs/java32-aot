...
public abstract class I386CompressAddressLowering extends I386AddressLowering {
    ...
    @Override
    protected final boolean improve(StructuredGraph graph, DebugContext debug, I386AddressNode addr, boolean isBaseNegated, boolean isIndexNegated) {
        if (super.improve(graph, debug, addr, isBaseNegated, isIndexNegated)) {
            return true;
        }

        if (!isBaseNegated && !isIndexNegated && addr.getScale() == I386Address.Scale.Times1) {
    ...
    private boolean tryToImproveUncompression(I386AddressNode addr, ValueNode value, ValueNode other) {
    ...
    protected abstract boolean improveUncompression(I386AddressNode addr, CompressionNode compression, ValueNode other);
