 ...
 * questions.
 */
package org.graalvm.compiler.lir.i386.phases;
...
import org.graalvm.compiler.lir.i386.I386Move.I386MultiStackMove;
...
/**
 * Replaces sequential {@link I386StackMove}s of the same type with a single
 * {@link I386MultiStackMove} to avoid storing/restoring the scratch register multiple times.
 *
 * Note: this phase must be inserted <b>after</b> {@link RedundantMoveElimination} phase because
 * {@link I386MultiStackMove} are not probably detected.
 */
public class StackMoveOptimizationPhase extends PostAllocationOptimizationPhase {
    ...
    private static class Closure {
        ...
        public void process(DebugContext debug, List<LIRInstruction> instructions) {
            for (int i = 0; i < instructions.size(); i++) {
                ...
                if (isStackMove(inst)) {
                    I386StackMove move = asStackMove(inst);
        ...
        private void replaceStackMoves(DebugContext debug, List<LIRInstruction> instructions) {
            int size = dst.size();
            if (size > 1) {
                I386MultiStackMove multiMove = new I386MultiStackMove(dst.toArray(new AllocatableValue[size]), src.toArray(new AllocatableValue[size]), reg, slot);
    ...
    private static I386StackMove asStackMove(LIRInstruction inst) {
        assert isStackMove(inst);
        return (I386StackMove) inst;
    }

    private static boolean isStackMove(LIRInstruction inst) {
        return inst instanceof I386StackMove;
    }
