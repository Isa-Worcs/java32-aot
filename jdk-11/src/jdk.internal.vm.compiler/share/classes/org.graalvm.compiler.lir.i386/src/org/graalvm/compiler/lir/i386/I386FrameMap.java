...
/**
 * I386 specific frame map.
 *
 * This is the format of an I386 stack frame:
 *
 * <pre>
 *   Base       Contents
 *
 ...
 */
public class I386FrameMap extends FrameMap {

    private StackSlot rbpSpillSlot;

    public I386FrameMap(CodeCacheProvider codeCache, RegisterConfig registerConfig, ReferenceMapBuilderFactory referenceMapFactory) {
    ...
    StackSlot allocateRBPSpillSlot() {
        ...
        rbpSpillSlot = allocateSpillSlot(LIRKind.value(I386Kind.QWORD));
        assert asStackSlot(rbpSpillSlot).getRawOffset() == -16 : asStackSlot(rbpSpillSlot).getRawOffset();
        return rbpSpillSlot;
    }

    void freeRBPSpillSlot() {
        int size = spillSlotSize(LIRKind.value(I386Kind.QWORD));
    ...
    public StackSlot allocateDeoptimizationRescueSlot() {
        assert spillSize == initialSpillSize || spillSize == initialSpillSize +
                        spillSlotSize(LIRKind.value(I386Kind.QWORD)) : "Deoptimization rescue slot must be the first or second (if there is an RBP spill slot) stack slot";
        return allocateSpillSlot(LIRKind.value(I386Kind.QWORD));
    }
}
