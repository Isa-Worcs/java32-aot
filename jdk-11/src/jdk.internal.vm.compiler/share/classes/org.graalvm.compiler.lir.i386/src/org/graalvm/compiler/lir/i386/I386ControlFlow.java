...
public class I386ControlFlow {

    public static final class ReturnOp extends I386BlockEndOp implements BlockEndOp {
    ...
    public static class BranchOp extends I386BlockEndOp implements StandardOp.BranchOp {
        ...
        protected void jcc(I386MacroAssembler masm, boolean negate, LabelRef target) {
    ...
    public static class StrategySwitchOp extends I386BlockEndOp {
        ...
        @Override
        public void emitCode(final CompilationResultBuilder crb, final I386MacroAssembler masm) {
        ...
        public class SwitchClosure extends BaseSwitchClosure {
            ...
            protected final I386MacroAssembler masm;

            protected SwitchClosure(Register keyRegister, CompilationResultBuilder crb, I386MacroAssembler masm) {
            ...
            protected void emitComparison(Constant c) {
                ...
                switch (jc.getJavaKind()) {
                    case Int:
                        ...
                        break;
                    case Long:
                        masm.cmpq(keyRegister, (I386Address) crb.asLongConstRef(jc));
                        break;
                    case Object:
                        I386Move.const2reg(crb, masm, asRegister(scratch), jc);
    ...
    public static final class TableSwitchOp extends I386BlockEndOp {
        ...
        @Override
        public void emitCode(CompilationResultBuilder crb, I386MacroAssembler masm) {
            Register indexReg = asRegister(index, I386Kind.DWORD);
            Register idxScratchReg = asRegister(idxScratch, I386Kind.DWORD);
            Register scratchReg = asRegister(scratch, I386Kind.QWORD);
            ...
            masm.leaq(scratchReg, new I386Address(I386.rip, 0));
            final int afterLea = masm.position();

            // Load jump table entry into scratch and jump to it
            masm.movslq(idxScratchReg, new I386Address(scratchReg, idxScratchReg, Scale.Times4, 0));
    ...
    @Opcode("SETcc")
    public static final class CondSetOp extends I386LIRInstruction {
    ...
    @Opcode("SETcc")
    public static final class FloatCondSetOp extends I386LIRInstruction {
    ...
    @Opcode("CMOVE")
    public static final class CondMoveOp extends I386LIRInstruction {
    ...
    @Opcode("CMOVE")
    public static final class FloatCondMoveOp extends I386LIRInstruction {
    ...
    private static void floatJcc(I386MacroAssembler masm, ConditionFlag condition, boolean unorderedIsTrue, Label label) {
    ...
    private static void cmove(CompilationResultBuilder crb, I386MacroAssembler masm, Value result, boolean isFloat, ConditionFlag condition, boolean unorderedIsTrue, Value trueValue,
                    Value falseValue) {
        // check that we don't overwrite an input operand before it is used.
        assert !result.equals(trueValue);

        I386Move.move(crb, masm, result, falseValue);
    ...
    private static void cmove(CompilationResultBuilder crb, I386MacroAssembler masm, Value result, ConditionFlag cond, Value other) {
        if (isRegister(other)) {
            assert !asRegister(other).equals(asRegister(result)) : "other already overwritten by previous move";
            switch ((I386Kind) other.getPlatformKind()) {
                case BYTE:
                case WORD:
                case DWORD:
            ...
            I386Address addr = (I386Address) crb.asAddress(other);
            switch ((I386Kind) other.getPlatformKind()) {
                case BYTE:
                case WORD:
                case DWORD:
    ...
    private static void setcc(I386MacroAssembler masm, Value result, ConditionFlag cond) {
